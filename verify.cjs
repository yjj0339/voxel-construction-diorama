...
const outputDirectory=path.join(root,'.verification'),temporaryDirectory=path.join(outputDirectory,'tmp');
...
(async()=>{
 const browser=await chromium.launch({executablePath,headless:true,args:['--allow-file-access-from-files','--disable-background-timer-throttling']});
 ...
 await page.screenshot({path:path.join(outputDirectory,'preview-day.png')});
 ...
 if(process.argv.includes('--full')){
  await page.evaluate(()=>__sandbox.setHour(23));await page.waitForTimeout(500);await page.screenshot({path:path.join(outputDirectory,'preview-night.png')});
  await page.keyboard.press('Space');await page.waitForTimeout(3000);await page.screenshot({path:path.join(outputDirectory,'preview-rain.png')});
  ...
  const result={initial,rainPerformance,interactions,safety,performanceStats,errors,network:requests.filter(u=>/^https?:/.test(u))};fs.writeFileSync(path.join(outputDirectory,'verification.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
 }
 await browser.close();

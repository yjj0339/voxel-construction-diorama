...
function mat(color){if(color&&color.isMaterial)return color;if(!mats.has(color))mats.set(color,new THREE.MeshStandardMaterial({color,roughness:.73,metalness:.05}));return mats.get(color);}
...
let instanceCount=0;
function flushBatches(){for(const[parent,records]of batches){const partitions=new Map();for(const r of records){const key=r.color&&r.color.isMaterial?r.color:baseMaterial;if(!partitions.has(key))partitions.set(key,[]);partitions.get(key).push(r);}for(const[m,rs]of partitions){const mesh=new THREE.InstancedMesh(cubeGeo,m,rs.length);rs.forEach((r,i)=>{mesh.setMatrixAt(i,r.matrix);if(!(r.color&&r.color.isMaterial))mesh.setColorAt(i,new THREE.Color(r.color));});mesh.castShadow=true;mesh.receiveShadow=true;mesh.instanceMatrix.needsUpdate=true;if(mesh.instanceColor)mesh.instanceColor.needsUpdate=true;mesh.computeBoundingSphere();parent.add(mesh);instanceCount+=rs.length;}}batches.clear();}
...
function label(parent,text,x,y,z,w,h,o={}){const bg=o.bg==null?'#23393d':o.bg,fg=o.fg==null?'#eadfbf':o.fg,fontSize=o.fontSize==null?Math.min(Math.max(64,Math.round(512*h/w))*.5,512/(text.length*.65)):o.fontSize;const tex=canvasTexture(512,Math.max(64,Math.round(512*h/w)),(c,cw,ch)=>{c.fillStyle=bg;c.fillRect(0,0,cw,ch);c.strokeStyle=fg;c.globalAlpha=.4;c.strokeRect(7,7,cw-14,ch-14);c.globalAlpha=1;c.fillStyle=fg;c.textAlign='center';c.textBaseline='middle';c.font=`600 ${fontSize}px "Segoe UI","Microsoft YaHei",sans-serif`;c.fillText(text,cw/2,ch/2);});const m=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshStandardMaterial({map:tex,roughness:.85,side:THREE.DoubleSide}));m.position.set(x,y,z);m.rotation.y=o.rotationY||0;parent.add(m);return m;}
...
function buildTable(){
 ...
 for(let i=0;i<380;i++){const x=-10.75+rand()*6.5,z=-3.95+rand()*6.95,s=.07+rand()*.14;if((x>-10.5&&x<-8.5&&z>-.9&&z<.9)||(x>-6.6&&x<-5&&z>-2.15&&z<-.85))continue;box(world,soil[i%5],x,.55+s/2,z,s,s,s);const queued=batches.get(world);queued[queued.length-1].loose=true;}
 ...
 for(let i=0;i<650;i++){let x=-14.7+rand()*29.4,z=-9.7+rand()*19.4;if((Math.abs(x)>11.05&&Math.abs(x)<13.95)||(Math.abs(z)>5.8&&Math.abs(z)<8.43)||(x>-11.2&&x<-3.9&&z>-4.4&&z<3.6)||(x>-9&&x<-4.7&&z>4.15&&z<5.55))continue;const s=.035+rand()*.06;box(world,[0xa98e69,0xddd0ab,0x857b68][i%3],x,G+s/2,z,s,s*.6,s);const queued=batches.get(world);queued[queued.length-1].loose=true;}
 buildDeskProps();
}
